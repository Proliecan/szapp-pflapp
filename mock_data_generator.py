...
from server.water_values import add_new_value

time_delta = 14     # Amount of days that the history goes back
measure_period = 3  # Period of water level measurements in hours
sine_weight = 9     # Sets the weight of the sine curve in the data (bigger value -> more curvy)
...
def generate(time_delta, measure_period, sine_weight):
    ...
    data = inverse_sawtooth(frequency=a, samplerate=sample_rate, amplitude=50, phaseshift=r.random()*1e6)
    sin = [(8*math.sin(i*(a/20))) for i in range(sample_rate)]
